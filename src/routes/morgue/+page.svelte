<script lang="ts">
  ...
  // Training Debrief State
  const isDebrief = $derived($trainingStore.phase === "debrief");
  let showDebriefMessage = $state(false);
  let showWelcome = $state(false);

  $effect(() => {
    if (isDebrief && !showDebriefMessage) {
      // Start Debrief Sequence
      setTimeout(() => {
        showDebriefMessage = true; // Show Overlay
        vibrate(HapticPatterns.Heavy);
        playLoad();

        // No auto-dismiss. User must click "VIEW LEGACY"
      }, 500);
    }
  });

  function handleViewLegacy() {
    unlockAudio();
    completeOnboardingOptimistic();
    trainingStore.complete();
    showDebriefMessage = false;
  }
...
{#if showDebriefMessage}
  ...
  >
    ...
    <div class="mb-8 relative">
      <div
        class="absolute inset-0 bg-red-600 blur-[40px] opacity-40 rounded-full animate-pulse"
      ></div>
      <svg
        class="w-32 h-32 text-white relative z-10 drop-shadow-[0_0_15px_rgba(220,38,38,0.8)]"
        viewBox="0 0 318 412"
        fill="#ff0000"
      >
        <path
    ...
    <!-- First Blood Stamp -->
    <div class="mb-12 rotate-[-5deg]">
      <h1
        class="text-6xl text-kl-crimson font-black tracking-tighter border-[6px] border-kl-crimson px-6 py-2 uppercase"
        style="text-shadow: 0 0 20px rgba(220,38,38,0.5); box-shadow: 0 0 20px rgba(220,38,38,0.3), inset 0 0 20px rgba(220,38,38,0.3);"
      >
        FIRST
        <br />
        BLOOD
    ...
    <div class="text-center mb-16 space-y-2">
      <p class="text-neutral-400 text-xs tracking-[0.2em] font-bold">
        CONTRACT EXECUTED.
      </p>
      <p class="text-neutral-200 text-xl tracking-widest font-bold">
        BODY COUNT: <span
          class="text-kl-crimson drop-shadow-[0_0_10px_rgba(220,38,38,0.8)]"
          >1</span
        >
      </p>
    </div>
    ...
    <button
      ...
      class="px-8 py-3 bg-transparent border border-kl-gold text-kl-gold hover:bg-kl-gold hover:text-black transition-all duration-300 text-sm tracking-[0.2em] font-bold uppercase shadow-[0_0_20px_rgba(212,175,55,0.1)]"
    >
      [ VIEW LEGACY ]
    </button>
  </div>
