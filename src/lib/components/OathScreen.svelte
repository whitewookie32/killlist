<script lang="ts">
  ...
  const HOLD_DURATION = 500; // 0.5 seconds to complete oath
  let holdStartTime = 0;
...
>
  ...
  <h1
    ...
    style="color: rgba(212, 175, 55, {0.6 +
      holdProgress * 0.4}); font-family: 'JetBrains Mono', monospace;"
  >
    THE ORDER.
  </h1>
  ...
  <div class="relative w-32 h-32 mb-16">
    ...
    <div
      ...
      style="
        background: linear-gradient(135deg, rgba(80, 20, 20, {0.3 +
        holdProgress * 0.4}) 0%, rgba(40, 10, 10, {0.2 +
        holdProgress * 0.3}) 100%);
        border-color: rgba(153, 0, 0, {0.3 + holdProgress * 0.4});
        box-shadow: 0 0 {holdProgress * 40}px rgba(153, 0, 0, {holdProgress *
        0.5});
      "
    >
      <!-- Grid texture -->
    ...
    <svg
      ...
      style="filter: drop-shadow(0 0 {holdProgress *
        20}px rgba(220, 38, 38, {holdProgress * 0.8}));"
    >
      <path
    ...
    {#if isCompleted}
      <div
        class="absolute inset-0 bg-kl-crimson rounded-lg animate-pulse opacity-30"
      ></div>
    {/if}
  </div>
  ...
  <p
    ...
    style="color: rgba(139, 115, 85, {0.5 +
      holdProgress * 0.5}); font-family: 'JetBrains Mono', monospace;"
  >
    {#if isCompleted}
      Oath accepted
    {:else}
