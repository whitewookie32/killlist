<script lang="ts">
  ...
  import { registryCount, openCount } from '$lib/stores/contracts';
      ...
      >
        ...
        <div class="relative">
          {#if item.icon === 'registry'}
            ...
            {#if $registryCount > 0}
              <span class="absolute -top-1 -right-2 min-w-[16px] h-4 px-1 bg-kl-gold/20 border border-kl-gold/40 text-[9px] text-kl-gold flex items-center justify-center rounded-full">
                {$registryCount}
              </span>
            {/if}
          {:else if item.icon === 'active'}
            ...
            {#if $openCount > 0}
              <span class="absolute -top-1 -right-2 min-w-[16px] h-4 px-1 bg-kl-crimson/80 text-[9px] text-white flex items-center justify-center rounded-full">
                {$openCount}
              </span>
            {/if}
        ...
        <!-- Active indicator -->
        {#if active}
          <div class="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-kl-gold"></div>
        {/if}
      </a>
    {/each}
  </div>
